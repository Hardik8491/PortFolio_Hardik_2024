...
const ExperienceItem = ({ data }: { data: IExperienceItem }) => {
    return (
        <CardBox className='p-4 items-center text-center bg-[var(--textColor10)] group min-h-auto'>
            <Column className='items-center justify-between w-full h-full gap-12'>
                <Column className='items-center justify-start'>
                    <FontAwesomeIcon
                        icon={faTrophy}
                        className='text-3xl/6 md:text-4xl/6 text-[var(--primaryColor)]'
                    />
                      
                    <p className='text-lg/6 font-semibold mt-4'>
                        {data.designation}
                    </p>
